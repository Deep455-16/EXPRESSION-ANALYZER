...
function showResult(data) {
  ...
  resultCont.innerHTML = `
    ...
    return `<div class="emotion-bar-row">
      ...
      ${faces.length > 0 ? `
      <div style="margin-top:16px;text-align:center;font-size:0.8rem;color:var(--text-secondary)">
        <strong>Faces detected:</strong>
        <div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center;margin-top:8px;">
          ${faces.map((f, i) => `
            <span style="background:rgba(255,255,255,0.05);padding:2px 8px;border-radius:4px;border:1px solid rgba(255,255,255,0.1)">
              Face ${i+1}: <span style="color:var(--primary)">${f.method || 'face-api'}</span>
            </span>
          `).join('')}
        </div>
      </div>
