...
const TemporalSmoother = {
  buffers: {},
  update: function(pid, face_emotions, facesDetected) {
    const now = Date.now();
    if (!this.buffers[pid]) this.buffers[pid] = { buffer: [], lastTs: now };
    const state = this.buffers[pid];
    
    if (facesDetected === 0) {
      if (now - state.lastTs > 2000) state.buffer = [];
      return;
    }
    state.lastTs = now;
    
    for (const fe of face_emotions) {
      if (!fe.scores) continue;
      state.buffer.push(fe.scores);
      if (state.buffer.length > 8) state.buffer.shift();
      
      const smoothed = {};
      const alpha = 0.3;
      for (const s of state.buffer) {
        for (const [em, val] of Object.entries(s)) {
          smoothed[em] = (smoothed[em] === undefined) ? val : alpha * val + (1 - alpha) * smoothed[em];
        }
      }
      
      let total = 0;
      for (const v of Object.values(smoothed)) total += v;
      if (total === 0) total = 1;
      
      let maxEm = 'neutral', maxVal = -1;
      for (const [em, val] of Object.entries(smoothed)) {
        smoothed[em] = val / total;
        if (smoothed[em] > maxVal) { maxVal = smoothed[em]; maxEm = em; }
      }
      
      fe.smoothed_scores = smoothed;
      fe.smoothed_emotion = maxEm;
      
      // smooth only primary face per participant
      break;
    }
  }
};
...
function handleResult(data) {
  ...
  TemporalSmoother.update(State.participantId, faces, faces.length);
  ...
  if (faces.length > 0 && State.showOverlay && ctx) {
    if (data.annotated_frame) {
      ...
      ctx.clearRect(0, 0, overlay.width, overlay.height);
      const source = data.source || (State.useBackend ? 'backend' : 'client');
      faces.forEach(fe => {
        const [x, y, w, h] = fe.bbox;
        const displayEm = (State.useSmoothed && fe.smoothed_emotion) ? fe.smoothed_emotion : fe.emotion;
        const displayConf = (State.useSmoothed && fe.smoothed_scores) ? fe.smoothed_scores[displayEm] : fe.confidence;
        
        const cfg = EMOTION_CONFIG[displayEm] || EMOTION_CONFIG.neutral;
        ...
        const label = `${displayEm.toUpperCase()} ${pct}%`;
        ...
        // F3 Source/Method Badge
        const method = fe.method || 'face-api';
        const badgeText = `${source} • ${method}`;
        ctx.font = 'bold 10px Inter, sans-serif';
        const btm = ctx.measureText(badgeText);
        const bg = source === 'backend' ? '#10b981' : source === 'client' ? '#60a5fa' : '#94a3b8';
        ctx.fillStyle = bg;
        ctx.fillRect(x, y + h + 4, btm.width + 8, 16);
        ctx.fillStyle = '#fff';
        ctx.fillText(badgeText, x + 4, y + h + 15);
      });
    }
  } else if (ctx) {
    ctx.clearRect(0, 0, overlay.width, overlay.height);
  ...
  if (faces.length > 0) {
    ...
    const displayEm = (State.useSmoothed && primary.smoothed_emotion) ? primary.smoothed_emotion : primary.emotion;
    const displayConf = (State.useSmoothed && primary.smoothed_scores) ? primary.smoothed_scores[displayEm] : primary.confidence;
    const displayScores = (State.useSmoothed && primary.smoothed_scores) ? primary.smoothed_scores : primary.scores;
    ...
    if (emotionConf) emotionConf.textContent = `${Math.round(displayConf * 100)}% confidence`;

    renderEmotionBars(displayScores);
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Smoothing toggle
  const smCheck = $('useSmoothed');
  if (smCheck) smCheck.addEventListener('change', () => {
    State.useSmoothed = smCheck.checked;
  });
